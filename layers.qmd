...
In the above example, the `text` argument specifies the name of a variable that will be used for text labels.
...
The `tm_text()` function has many arguments that allow us to adjust the appearance of text labels.
...
@fig-tmtext-2 shows an example of a map with text labels moved a bit to the right and down, and rotated by 12 degrees.
...
#| fig-subcap:
#|   - "Default text labels"
#|   - "Customized text label position"
<<tmtext1>>
<<tmtext2>>
```
...
It includes a set of arguments that allow for the adjustment of the text labels' appearance, such as `shadow`, `just`, and `remove_overlap` (@fig-tmtext2).
