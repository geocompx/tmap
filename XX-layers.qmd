...
elev_isopleths = as.contour(slo_elev_raster)
...
```{r}
library(stars)
sat = read_stars("data/slovenia/park_sat2c.tif")
```
...
#| eval: false
tm_shape(sat) +
  tm_rgb(tm_vars(dimvalues = c("R", "G", "B"), n = 3, multivariate = TRUE),
...
tm_shape(sat) +
  tm_rgb(tm_vars(dimvalues = c("N", "R", "G"), n = 3, multivariate = TRUE),
         col.scale = tm_scale_rgb(stretch = TRUE, probs = c(0.02, 0.98)))
```
