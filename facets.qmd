...
slo_regions_ts = subset(slo_regions_ts, 
                        time %in% c("2006", "2010", "2014", "2018", "2022"))
```
...
This function creates a grid of panels, each showing the data for a specific value of the variable (here `"time"`), and also allows specifying the layout of the facets (e.g., number of columns) (@fig-facets2).
...
## Importance of layers order for faceting
...
\index{tm\_facets}
\index{tm\_facets\_wrap}
\index{tm\_facets\_grid}
...
<!-- mention the rest -->
<!-- Martijn, should we add some content of any other of the facets type? -->
...
<!-- Martijn, should it not be a part of the tm_facets function? -->
...
Next, we create facets for the raster data using `tm_facets()` and set the number of columns to three -- this results in a grid of panels, each showing the average temperature for a specific month.
Default layer names are usually not very informative, so we can replace them with more descriptive labels using a vector of labels provided to the `panel.labels` argument of the `tm_layout()` function, e.g., `panel.labels = c(1:12)` (@fig-facets10).
...
In the example below, we also customize the map further by updating the color scale to use the "viridis" palette, removing the title from the legend and adding a title to the map, improving the panel labels, and adjusting the inner margins of the facets to make them more readable.
...
For example, `col.free = c(TRUE, FALSE)` means that the color scale is applied freely across rows of facets, but shared across columns.
:::
