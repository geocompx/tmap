...
Here, we not only change the legend orientation, but also customize its position <!--ref position chapter--> with `tm_pos_out()` ^[By default, the `"landscape"` legend is located in the bottom right outside of the map frame].
The three arguments used, `"center"`, `"top"`, and `"center"` specify the legend position in horizontal and vertical position outside the map frame and then its position relative to that placement.
...
To disable the legend, we can set the `show` argument of the `tm_legend()` function to `FALSE`.
