...
The `tm_layout()` function has also tools to modify the the complete map color style with arguments such as `color.saturation`, `color.sepia_intensity`, and `color_vision_deficiency_sim`.
The first one represents the saturation of all of the colors on the map, including the backgrounds, but also the visual variables, such as fill or color of polygons and lines.
...
The `color_vision_deficiency_sim` has a different purpose: it allows us to see how the map would look like for people with different color vision deficiencies.
...
Its style depends on a several arguments of the `tm_layout()` function.
...
Another arguments that can be used to customize the frame are `frame.r` and `frame.double_line`  (@fig-layout-frame-2).
...
Maps consist of various pieces, such as the map content (with its frame), additional map elements (e.g., a credits or title), and the legend located in different places.
They also have many margins -- spaces between data and the frame, spaces between the frame and the plotting spaces between the icons and the labels, etc.
...
Aspect ratio specifies the relation between the width and height of the map area of the plot.
...
For maps, this is not the case -- we should not change the relation between the width and height of the data in a map as it has a specific projection which determines the spatial relation between the map features.
