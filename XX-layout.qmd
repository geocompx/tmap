...
tm = tm_shape(slo_borders) +
    ...
    tm_title("Population density in Slovenia")
tm
```

## Scale and aspect {#sec-scale-aspect}
...
idea: have tm1 and tm

<!-- update colors -->
```{r}
tm + 
  tm_layout(bg.color = "red",
            outer.bg.color = "pink")
```
...
```{r}
tm + 
  tm_layout(color.saturation = 0.6)
```
...
<!-- explain why/when customization of the margins is needed -->
