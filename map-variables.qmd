...
# Map variables {#sec-map-variables}

\index{map variables}
Map variables are methods to translate information given in variables into many types of visualizations, including maps.
Basic map variables are color, size, and shape^[Other map variables include position, orientation, and texture.].
...
The use of map variables on maps depends on two main things: (a) the type of the presented variable, and (b) the type of the map layer (@sec-const-map-variables; @sec-auto-scales).

<!-- alpha? -->
```{r}
#| label: fig-map-variables
#| echo: false
#| warning: false
#| message: false
#| fig-cap: Basic map variables and their representations on maps
#| fig-asp: 0.4
tmap::tmap_options(scale = 0.75)
source("code/visual_variables.R")
visual_variables()
tmap::tmap_options(scale = 1)
```

\index{color}
\index{size}
\index{shape}
@fig-map-variables shows examples of different map variables.
Color is the most universal map variable (@sec-colors).
...
Finally, there are some cases in which we can use several map variables at the same time (@sec-mixing-map-variables).
...
## Constant map variables {#sec-const-map-variables}

Before we use map variables, it is worth reminding that we can set constant values for map variables: fill, color, size, and shape for all objects on the map.
...
\index{tm\_scale}
On the other hand, if we provide the name of the column (variable) we want to visualize, **tmap** automatically sets the map variables.
...
In this and the following sections, we show how to use the `tm_scale()` function to modify the appearance of the map variables.
...
This is done with the `values` argument of each of these functions, which can be one of many things, depending on the map variable.
...
#| fig-cap: "Examples of various ways of specifying values for map variables."
#| fig-subcap: 
#|    - name of a build-in color palette
#|    - vector of colors
#|    - numerical variable for sizes
#|    - numerical variable for shapes
#| message: false
#| layout-ncol: 2
tm_shape(slo_cities) +
...
The following sections @sec-colors, @sec-sizes, and @sec-shapes explain the basic map variables.
...
#| echo: false
#| fig-cap: "Examples of three main types of color palettes: categorical, sequential, and diverging."
#| fig-asp: 0.5
...
#| fig-subcap:
#|   - default sequential color palette
#|   - diverging color palette around the midpoint value of 73
#| fig-height: 2
<<tmpals1>>
<<tmmidpoint2>>
```
...
```{r}
#| label: fig-tmlwd
#| fig-cap: "Example of a map where lines' widths represent values of the corresponding lines."
...
## Mixing map variables {#sec-mixing-map-variables}
...
Therefore, it is possible to mix some map variables for symbols and lines.
This section shows only some possible examples of mixing map variables.
...
We can also modify all of the map variables using the additional arguments explained in the next chapters.
...
#| message: false
#| layout-nrow: 2
#| fig-cap: 'Examples of maps using two map variables at the same time.'
#| fig-subcap: 
#|   - size and fill
#|   - fill and shape
<<mixsymb1>>
...
#| message: false
#| fig-asp: 0.66
#| fig-cap: A map using two map variables, color, and size (line width), at the same time.
tm_shape(slo_railroads) +
