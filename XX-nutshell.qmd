...
For example, we read the `slo_cities.gpkg` file containing several points on Easter Island into a new object, `slo_cities`, and select only points with the `type` of `"volcano"`.
...
Let's see how many *groups* can be used by reading some additional datasets -- the `slo_elev` raster with elevation data for Easter Island, the `slo_borders` polygon with the island outline, and the `slo_railroads` lines contains a road network for this island.
<!-- ref data appendix -->
...
The second *group* shows the borders of Easter Island with the default aesthetics, while the third *group* presents the road network (the `slo_railroads` object), with each line's width based on the values from the `"strokelwd"` column, but with a legend hidden.
