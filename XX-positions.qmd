...
In general, there are eight locations for map components outside the map frame -- each of them can be specified with `tm_pos_out()`.
...
- `align.h` and `align.v`: the alignment of the components is only used when many components are grouped together. <!--ref to the next section?-->
- `just.h` and `just.v`: the justification of the map components only used when `pos.h` and `pos.v` are specified as numbers between 0 and 1.
