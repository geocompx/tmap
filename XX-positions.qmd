...
The `align.h` and `align.v` arguments are used when many map components are grouped together, which is explained in [@sec-grouping].
...
- `align.h` and `align.v`: the alignment of the components is only used when many components are grouped together (@sec-grouping).
...
Various map components can be either positioned at the same time.
For example, we can place a scale bar outside the map frame and a legend inside the map frame <!--ref-->.
