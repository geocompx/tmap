...
This chapter focuses on the map projections.
It gives a background on why do we need map projections and how to translate spatial data from an ellipsoid into a flat surface or computer screen.
...
knitr::kable(tb2,
  ...
  kableExtra::column_spec(c(3,5), width = "14em")
```
...
knitr::kable(tabcrsrec, booktabs = TRUE) |> 
  kableExtra::kable_styling(font_size = 12) |> 
...
We will expand on the short introduction from @sec-map-projection-intro and show how to use various tools available in the `tm_crs()` function.

### Vector data

First, see how to specify a map projection for vector data.
We will use the `worldvector.gpkg` and `worldcities.gpkg` datasets -- the first one uses the Equal Earth projection (`EPSG:8857`), and the second one uses the WGS 84 coordinate system (`EPSG:4326`).
Additionally, to show the effect of map projections, we will transform the `worldvector` data into the WGS 84 coordinate system, namely `worldvector_wgs84`.
...
The `tm_crs()` function is actually a small toolbox helping to manage map projections.
...
It will automatically select a suitable projection based on the data's extent and the map's properties.
For example, if the map is global, it will select the pseudocylindrical projection Equal Earth (@fig-mproj-auto).
...
Then, we can also set the `bg.color` argument to specify the background color of the map, which will only apply to the area inside the Earth boundary for a given projection (@fig-mproj3).
<!-- should we mention tm + tm_style("natural") ?-->
...
You may notice that these projections have not only different visual styles but also different shapes of the Earth boundary and focus on preserving different map projection properties.
