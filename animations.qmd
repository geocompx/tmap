...
- `tmap_animation()`, which saves the animation to files in graphical and video formats, such as GIF, MP4, and WebM
...
```{r}
#| label: fig-anims
#| fig-cap: "Animations of GDP per capita in Slovenian regions over time"
#| fig-subcap:
#|   - "Using `tm_animate()`"
#|   - "Using `tm_animate_fast()`"
#| echo: false
tm +
  tm_animate(by = "time")
tm +
...
either `"loop"` (default), which plays the animation in a loop, `"once"`, which plays the animation only once and then stops, or `"pingpong"` which plays the animation back and forth.
