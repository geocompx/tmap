...
tm_shape(worldelevation, bbox = tmaptools::bb("Europe")) +
  tm_raster(
    col.scale = tm_scale_continuous(values = "geyser", midpoint = NA,
                                    limits = c(-500, 6500))
  ) +
  tm_title("Slovenia", position = c("RIGHT", "CENTER")) +
  tm_inset(tm_slo_elev, position = c("RIGHT", "CENTER")) +
  tm_crs("+proj=eqearth", bbox = tmaptools::bb("Europe")) +
...
gg1 = ggplot(slo_cities, aes(place)) +
  geom_bar() +
  scale_y_discrete() +
  labs(x = NULL, y = NULL) +
  theme_bw()
