...
Let's start by attaching the packages and reading the data we will use in this chapter.
...
On the other hand, if we provide the name of the column (variable) we want to visualize, **tmap** will automatically set the visual variables.
...
In the next two chapters, we will further discuss various types of scales and ways to modify them and the legend appearance.
...
Therefore, `"-yl_gn"` return a palette going from green to yellow.
