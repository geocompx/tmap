...
It is also possible to reverse the order of any named color palette by using the `-` prefix^[In general, the darker the color, the larger the value it represents. This rule applies to sequential and diverging color palettes, when the background color is light. However, when the background color is dark, the rule is reversed, and the darker colors should represent lower values.].
