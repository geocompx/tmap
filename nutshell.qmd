...
Here, we specify the input data (our *shape object*) with `tm_shape()`, map variables (also known as *aesthetics*) of map layers with `tm_symbols()`, and the map title with `tm_title()`.
...
Overall, most map variables of map layers can be assigned in two main ways.
...
You can also notice that we can control scales of various map variables, such as color, size, or width, with the `tm_scale_*()` function and customize legends with the `tm_legend()` function.
...
One way to visualize this data as facets is to specify selected columns as map variables in the map layer function (e.g., `tm_polygons()`) (@fig-sec-facets1).
